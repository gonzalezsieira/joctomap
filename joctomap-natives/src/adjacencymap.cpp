...
JNIEXPORT void JNICALL Java_es_usc_citius_lab_joctomap_util_AdjacencyMap_initializeJNI
  (JNIEnv *env, jobject jadjacencymap, jobject joctree){
    ...
    if(octree->size() > 0) {
        ...
        for (OcTree::leaf_bbx_iterator it = octree->begin_leafs_bbx(point3d(minx, miny, minz), point3d(maxx, maxy,
                                                                                                       maxz)), end = octree->end_leafs_bbx();
             it != end; ++it) {
            ...
            jobject jpoint3d = env->NewObject(cls_point3d, constructor_point3d, coordinate.x(), coordinate.y(),
                                              coordinate.z());
            ...
            for (OcTree::leaf_bbx_iterator it2 = octree->begin_leafs_bbx(
                    point3d(coordinate.x() - diff, coordinate.y() - diff, coordinate.z() - diff),
                    point3d(coordinate.x() + diff, coordinate.y() + diff,
                            coordinate.z() + diff)), end2 = octree->end_leafs_bbx(); it2 != end2; ++it2) {
                ...
                if (key != key2
                    ...
                    //build object "joctreekey"
                    jobject joctreekey2 = env->NewObject(cls_joctreekey, constructor_joctreekey, key2.k[0], key2.k[1],
                                                         key2.k[2]);
